...
public static class Extensions
{
    ...
    {
        if (!data.Values.TryGetValue(key, out var value)) return defaultValue;
    ...
    public static Vector2 Vector(this EntityData data, string name) //thanks snip!!!!@!!!!!! :333   -Aletris
    {
        string[] splitValue = data.Attr(name).Split(',');

        if (splitValue.Length != 2)
        {
            foreach (string v in splitValue)
                Util.Log(LogLevel.Error, v);
        ...
        if (!int.TryParse(splitValue[0], out var x))
            throw new InvalidOperationException($"\"{name}\" is not a valid int; \"{splitValue[0]}\" (X component) is not a number");

        if (!int.TryParse(splitValue[1], out var y))
            throw new InvalidOperationException($"\"{name}\" is not a valid int; \"{splitValue[1]}\" (Y component) is not a number");
